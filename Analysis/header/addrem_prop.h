// This file considers proposals to add and remove individuals from the system
...
void Chain::addrem_prop()                // Adds and remove indiduals from the population
{
  ...
  if(ran() < 0.5){     // Adds an individual
    ...
    z = ran()*addremdivsumtot; 
		a = 0; while(a <= naddremdiv && z > addremdivsum[a]) a++;
		if(a > naddremdiv) emsg("addrem: EC1");
    ...
    z = ran()*induoagesumtot[a]; 
		ag = 0; while(ag < nclassval[agecl] && z > induoagesum[a][ag]) ag++; 
		if(ag == nclassval[agecl]) emsg("addrem: EC2");
    probif += loginduoage[a][ag];

    if(tbirthfl == 1){
      if(ag == 0){ tbirth = tent - ran()*age[ag]; probif += log(1.0/age[ag]);}
      else{ 
				tbirth = tent - (age[ag-1] + ran()*(age[ag]-age[ag-1])); 
				probif += log(1.0/(age[ag]-age[ag-1]));
			}
      dLpri = agprior[ag];
    }
    ...
    if(sourcefixfl == 1){
      tr = evnew[0].tr; 
			if(tra[tr].capev >= 0 && capevall[tra[tr].capev] == 1){ nfa_add++; rememptyind(i); return;}
    }
    if(sinkfixfl == 1){
      tr = evnew[evnew.size()-1].tr; 
			if(tra[tr].capev >= 0 && capevall[tra[tr].capev] == 1){ nfa_add++; rememptyind(i); return;}
    }
    ...
    al = exp(Ltotf - Ltoti + probfi - probif + dLpri); 
    ...
    if(ran() < al){
      ...
      if(samp < burnin){    //  Tunes proposals
        ...
        c = tra[indev[i][0].tr].cf%ncomps; if(c == NOTALIVE) emsg("addrem: EC3");
...
void Chain::addreminit()              // Initialising proposals for adding and removing individuals
{
  ...
  induoagesumtot.resize(naddremdiv+1); induoage.resize(naddremdiv+1); induoagesum.resize(naddremdiv+1); 
	loginduoage.resize(naddremdiv+1);
  ...
  for(a = 0; a <= naddremdiv; a++){
    ...
    induoage[a].resize(nclassval[agecl]); induoagesum[a].resize(nclassval[agecl]); 
		loginduoage[a].resize(nclassval[agecl]);
