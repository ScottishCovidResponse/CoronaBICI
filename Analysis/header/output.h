...
void Chain::diagnosticsfile()       // Outputs a file giving MCMC diagnostic information
{
  ...
  dia << "MULTIMOVE\n";
	...
	for(c = 0; c < ntramm; c++){
		if(ntr_singfix[c] > 0){ 
			v = nac_singfix[c]/ntr_singfix[c]; 
			f += v; nf++; if(v > fmax) fmax = v; if(v < fmin) fmin = v;
		}
	}
	dia << "ac: " << f/nf << " (" << fmin << "-" << fmax << ")\n";
  ...
  dia << "Paramameters: " << long(100*timeprop[PARAM_PROP]/totaltime) << "%\n";
	...
	dia << "Singfix: " << long(100*timeprop[SINGFIX_PROP]/totaltime) << "%\n";
...
void tracefileinit()                                 // Initialises the trace file
{
	...
	for(r = 0; r < nclassval[1]; r++){
		trace << "inf" << classval[1][r] << "\t";
	}
	
	for(s = 0; s <= nsettime; s++){
		trace << "inf" << classval[settimecl][s] << "\t";
	}
	trace << "inftot\t" << "Li\t";
	
	for(r = 0; r < nclassval[1]; r++) trace << "multacf" << classval[1][r] << "\t";	 
	trace << "end\n";
...
void Chain::loadsimulated()
{
	...
	for(i = 0; i < nindtot_sim; i++){
		...
		indinit[i] = c;
		nindfixev[i] = 0; indfixev[i].clear();
    ...
    for(e = 0; e < nev; e++){
			...
			if(tra[tr].cl == settimecl){
				for(s = 0; s <= nsettime; s++){
					if((t-settime[s])*(t-settime[s]) < 1) t = settime[s];
				}
			}
			
		 if(tra[tr].cf == NOTALIVE) t = tmax;
			EV ev; ev.t = t; ev.tr = tr;
			evnew.push_back(ev);
				
			if(tra[tr].cl == 0 && tra[tr].i != tra[tr].f && tra[tr].f == HH){
				indfixev[i].push_back(fixev.size());
				nindfixev[i]++;
				 
				cl = 0;
				FEV fev; 
				fev.i = i; fev.t = t; fev.cl = cl; 
				fev.trai = tra[tr].i; fev.traf = tra[tr].f; fev.capev = 0;
				fev.dc = (tra[tr].f-tra[tr].i)*classmult[cl]; fev.type = tra[tr].type;
				fev.like = 1;
		...
		c = tra[evnew[long(evnew.size())-1].tr].cf;
		if(c != NOTALIVE){
	...
	cout << nfixev << " " << i << " " << Lir << " " << Liexp << " " << L() << " Load simulated done\n";
}
