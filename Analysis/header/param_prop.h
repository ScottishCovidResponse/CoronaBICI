// This file makes parameter MCMC proposals 
...
void Chain::param_prop()          // Makes random walk proposals on model parameters
{
  ...
  for(p = 0; p < nparam; p++){
    ...
    if(notinsideprior(p,param_new) != 2){
  ...
  for(s = 0; s < nsmooth; s++){    // Makes changes to all smooth parameters at the same time
    ...
    if(j < nsmoothparam[s]) al = -1;
      ...
      Ltoti = L();
      for(j = 0; j < nsmoothparam[s]; j++){
				p = smoothparam[s][j]; changeparam(1,p,param[p]+dparam);
			}
      Ltotf = L();
      al = exp(Ltotf-Ltoti);
    }
    ...
    if(ran() < al){
      ...
      if(al >= 0){ 
				for(j = 0; j < nsmoothparam[s]; j++){ 
					p = smoothparam[s][j]; changeparam(0,p,param[p]-dparam);
				}
			}
  ...
  sum = 0; 
	for(c = 0; c < ncompswa; c++){ num[c] = gammasamp(ncompinit[c]+1+dirchal[c]-1,1); sum += num[c];}
...
void Chain::paramnorm_prop()        // Makes a proposal which is normally fitted to posterior
{
  long p, pr;
...
void Chain::param_gradprop()             // Makes proposals to increase posterior probability
{
  ...
  for(p = 0; p < nparam; p++){
    ...
    if(priortype[pr] != FIX){
      ...
      dLpri = 0;
      if(pr >= 0) dLpri -= priorprob(pr);
			for(j = 0; j < nparampriordep[p]; j++) dLpri -= priorprob(parampriordep[p][j]);
...
void Chain::paramgrad(long p)              // Calculates the gradient and curvature on a parameter
{
    ...
    eq = transdepeq[d];
		...
		if(discon == 1){
			for(kd = d*DX; kd < d*DX+DX; kd++){
				...
				dLup += -(valnewup-valold)*depeq_disc_evdt[kd] + (log(valnewup)-log(valold))*depeq_disc_evn[kd];
				dLdo += -(valnewdo-valold)*depeq_disc_evdt[kd] + (log(valnewdo)-log(valold))*depeq_disc_evn[kd];
			}
		}
		else{
			e = depeqdiv[d][0];
  ...
  for(j = 0; j < nparam_capev[p]; j++){
    ...
    if(capev_oneminus[d] > 0){ 
			dLup += (valnewup2-valold2)*capev_oneminus[d];
			dLdo += (valnewdo2-valold2)*capev_oneminus[d];
		}
  }
...
void paraminit()                 // Initialisation for parameter proposals
{
  ...
  for(k = 0; k < nnmeq; k++){ 
		eq = nmeq[k]; for(j = 0; j < neq_param[eq]; j++) param_nmfl[eq_param[eq][j]] = 1;
	}
  ...
  if(1 == 0){
		for(p = 0; p < nparam; p++){
			cout << paramname[p] << ": ";
			for(j = 0; j < nparam_dep[p]; j++) cout << eqnstr[transdepeq[param_dep[p][j]]] << ", ";
			for(j = 0; j < nparam_notdep[p]; j++) cout << eqnstr[transnotdepeq[param_notdep[p][j]]] << ", ";
			for(j = 0; j < nparam_cope[p]; j++) cout << eqnstr[cope[param_cope[p][j]]] << ", ";
			for(j = 0; j < nparam_cpe[p]; j++) cout << eqnstr[cpe[param_cpe[p][j]]] << ", ";
			cout << "\n";
		}
	}
}
