// This file deals with state observations on the model
...
void observationinit()                                   // Initialises the observation model
{
  ...
  for(cap = 0; cap < ncap; cap++){
    capindob.push_back(vector<short>());
...
double dercalc(short j, double *popnum, double *param)  // Observation probability for a derived quantity
{
  long d;
  d = derm[j];
