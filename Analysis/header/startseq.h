// This file generates a pausible initial event sequence consistent with data
...
double Chain::startseq(long i)
{
      ...
      for(cc = 0; cc < ncomps; cc++){
        ...
        if(val > 0){
          for(j = 0; j < cposto[cc].size(); j++){
            ...
            if(prob[e][c] < val2){ 
							if(fixfl== 0 || checkforfixedinpath(c,cc,ctopp) == 0) prob[e][c] = val2;
						}
          }
        }
      }
  ...
  if(1 == 0){
		if(indid[i] == "Ind. 4"){
			cout << indid[i] << " " << i <<  " id\n";
			for(e = 0; e < nevp; e++){
				cout << e << " " << evp[e].t << " "<<  evp[e].ty << " " << evp[e].ch << " evp\n";
			}
			
			for(e = 0; e < nevp-1; e++){
				cout << "evp " << e << ":\n"; 
				for(c = 0; c < ncomps; c++){
					if(prob[e][c] != 0) cout << compname[c] << ":" << prob[e][c] << ", ";
				}
				cout<< "\n";
			}
			emsg("Startseq: EC1");
  ...
  for(e = 0; e < nevp-1; e++){
    ...
    cs = c%ncomps;
		...
		// If no time then must be the same (e.g. observations at same time as age/time/fixed timelines)
    if(evp[e+1].t == evp[e].t) csf = cs;  
    else{
      for(j = 0; j < cposfr[cs].size(); j++){
        csf = cposfr[cs][j];
        val = prob[e+1][csf];
        if(val > 0){
          if(fixfl == 0 || checkforfixedinpath(cs,csf,ctopp) == 0){ 
						sum += val*cpos[cs][csf]; pos.push_back(csf); possum.push_back(sum);
					}
        }
      }
      if(sum == 0) return 0;
    ...
    for(cl = 0; cl < nclass-3; cl++){
      ...
      if(ti != tf){
        if(evp[e+1].t == evp[e].t) emsg("Startseq: EC2");
        si = path[cl][ti][tf].size(); if(si == 0) emsg("Startseq: EC3");
        for(k = 0; k < si; k++){
          tr = path[cl][ti][tf][k];
          PT pta;
					pta.t =  evp[e].t + (k+0.1+0.9*ran())*(evp[e+1].t -evp[e].t)/si;
					pta.dc = (trans[tr].f-trans[tr].i)*classmult[cl];
          pt.push_back(pta);
        }
      }
    }
  ...
  if(numwrong(i,evnew) > 0) emsg("Startseq: EC4");
 
  return probuocap(i,evnew);
}

long getctop(double t, double tbirth)  // Returns the compartment part related to the top three cl
{
  long ctop, ag, ti, f;
...
long checkforfixedinpath(long c, long cc, long ctopp)  // Checks if fixed events stop a path from occuring
{
      ...
      for(j = 0; j < nval; j++){
          ...
          if(j != jj && si > 0){
            for(k = 0; k < transleave[jj].size(); k++){
              ...
              if(jjj != j){
                ...
                if(sinew == 0 || si+1 < sinew){ 
									path[cl][j][jjj] = path[cl][j][jj]; path[cl][j][jjj].push_back(tr); fl = 1;
								}
              }
            }
          }
		...
		if(1 == 0){
			for(j = 0; j < nval; j++){
				for(jj = 0; jj < nval; jj++){
					cout << j << " " << jj << "  ";
					for(k = 0; k < path[cl][j][jj].size(); k++){
						cout << trans[path[cl][j][jj][k]].i << "," << trans[path[cl][j][jj][k]].f << "   ,"; 
					}
					cout << "\n";
				}
			}
  ...
  for(c = 0; c < ncomps; c++){
    ...
    for(cc = 0; cc < ncomps; cc++){
      ...
      if(cl < nclass-3) cpos[c][cc] = 0;
        ...
        if(notimerange == 1) cpos[c][cc] = exp(-100*num); // If no data range then make very unlikely
...
double Chain::probuocap(long i, vector<EV> &vec)  // The probability of captures which are not observed
{
      ...
      if(eq != -1){
        ...
        if(j == nindobs[i]){
          if(eqnstr[eq] == "1"){
            ...
            stringstream ss;
						ss << "Individual '" << indid[i] << "' should be observed in capture '" 
								<< capname[ca] << "' but it is not.";
            ...
            if(capwarn != "not"){
              if(capwarn == "") capwarn = ss.str();
                ...
                if(capwarn != ss.str()){
                  stringstream ss2; 
									ss2 << "Individual '" << indid[i] 
											<< "' should be observed in other captures but it is not.";
                  capwarn = ss.str();
                }
              }
            }
