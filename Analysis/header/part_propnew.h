// This file makes particle proposals for individual event sequences
...
void Chain::part_prop(long i)                      // Makes a particle proposal on an individual
{
  ...
  indsim_init(i,indtbirth[i],1);
	...
	// Divides up into sections defined by observations or fixed events
  nevpsec = 0; evpsec.clear(); evpref.clear();  
  k = 0; ee = 0;
  for(e = 0; e < nevp; e++){
    t = evp[e].t;
  ...
  for(li = 0; li < nevpsec; li++){
    evpbeg = evpsec[li].evpbeg; evpend = evpsec[li].evpend; 
		if(evpbeg == evpend) emsg("Part: EC1");
    pbeg = li*npart; pend = pbeg+npart;

    for(p = pbeg+1; p < pend; p++) part_simsec(i,evpbeg,evpend,part[p]);

    ty = evp[evpend].ty; ch = evp[evpend].ch;
    if((ty == END && ch == 1) || ty == FIXEV || ty == OTHCL){ 
			t = onefac*evpsec[li].tend; 
			k = evpref[evpend]; if(k == 0) co = NOTALIVE; else co = cexist[k-1];
		}
        ...
        switch(ty){
          ...
          case FIXEV:
            if(compval[c][fixev[ch].cl] != fixev[ch].trai) w = 0;
              ...
              if(fixev[ch].like == 1){
                cc = c+fixev[ch].dc; if(cc >= ncomp) emsg("Part: EC2");
                tr = compiftra[c][cc];
    ...
    if(li < nevpsec-1){
      if(numzero >= 0.5*npart){  // does a bootstrap
        ...
        if(wsum == 0){
          nfa_part[i][partcl]++;
          if(samp < burnin){ 
						nindpart[i][partcl] *= facup; 
						if(nindpart[i][partcl] >= npartmax) nindpart[i][partcl] = npartmax*onefac;
					}
          timeprop[PARTSIM] += clock();
          return;
        }
  ...
  z = ran()*wsum; p = pbeg+1; while(p < pend && z > wsumst[p]) p++; 
	if(p == pend) emsg("Part: EC4");
  wf = part[p].w;
  ...
  if(li != -1) emsg("Part: EC5");

  evnew.clear();
  
  for(li = 0; li < nevpsec; li++){
		p = psel[li]; nev = part[p].ev.size(); 
		for(e = 0; e < nev; e++) evnew.push_back(part[p].ev[e]);
	}
  ...
  al = exp(Ltotf - Ltoti + probfi - probif)*fac;
  ...
  if(ran() < al){
    ...
    if(samp < burnin){
      if(psel[0] != 0){
				nindpart[i][partcl] *= facdown; 
				if(nindpart[i][partcl] < 2) nindpart[i][partcl] = 2;
			}  
      else{ 
				nindpart[i][partcl] *= facup; 
				if(nindpart[i][partcl] >= npartmax) nindpart[i][partcl] = npartmax*onefac;
  ...
  if(samp < burnin){
    c = tra[indev[i][0].tr].cf; if(c == NOTALIVE) emsg("Part: EC7");
		...
		if(partcl == clall){
			...
			c = compval[c][partcl];
			if(indev[i][0].t == 0) simcl_initc[i][partcl][c] += initfac;
			else simcl_enterc[i][partcl][c] += initfac;
		}
  }
}
...
void Chain::insertexisting(vector<EV> &ev)     // Populates particles with the existing sequence
{
  ...
  for(li = 0; li < nevpsec; li++){
    ...
    while(e < nev && ev[e].t < tma){
        ...
        if(tra[tr].nm == 1 && !(tra[tr].capev >= 0 && capevall[tra[tr].capev] == 1)){
          ...
          if(partcl == clall && cl == partcl){
            ...
            if(tt < evpsec[li].tbeg){
              li2 = li-1; 
							while(li2 >= 0 && tt < evpsec[li2].tend){ addexistingfute(part[li2*npart],tr,tt,t); li2--;};
            }
          }
        }
        ...
        if(tra[tr].ntraend > 0){
          for(j = 0; j < tra[tr].ntraend; j++){
            ...
            if(tt < evpsec[li].tbeg){ // Need to sample from
              ...
              li2 = li-1; 
							while(li2 >= 0 && tt < evpsec[li2].tend){
								addexistingfute(part[li2*npart],tr,tt,ttt); li2--;
							};
            }
          }
        }
      ...
      c = tra[tr].cf; if(c == NOTALIVE) emsg("Part: EC10");
  ...
  if(e != nev-1) emsg("Part: EC11");

  if(checkon == 1){
    for(li = 0; li < nevpsec; li++){
      p = li*npart; e = evpsec[li].evpbeg; 
			if(evp[e].ty == CHC){ if(tra[part[p].ev[0].tr].dc != evp[e].ch) emsg("Part: EC12");}
    }
  }
}

void Chain::addexistingfute(PART &pa, long tr, double tbeg, double tend) // Adds exisiting future events
{
  long m;
  ...
  for(e = evpbeg; e < evpend; e++){
    ...
    if(t < tnext){
      do{
        ...
        if(nmfl == 1){ 
					if(nfuteref > 0){ 
						futnext = futev[futeref[nfuteref-1]].t;
						if(tt > futnext) tt = futnext;
					}
					else futnext = large;
				} 
				else futnext = large;
...
void Chain::chainpartinit()             // Initialises quantities used when sampling from particles
{
  ...
  long i;
	...
	if(corona == 0){
		...
		for(i = 0; i < nind; i++){
			nindpart[i].resize(clall+1); 
			for(cl = 0; cl <= clall; cl++) nindpart[i][cl] = npartstart;
		}
	}
	
  for(c = 0; c < ncomps; c++) enterpos[c] = 0; // Works out if ind can enter into a particular compartment
  for(c = 0; c < ncomp; c++) if(compiftra[NOTALIVE][c] >= 0) enterpos[c%ncomps] = 1;

  if(corona == 0){
		sim_initcsumtot.resize(nind); sim_initc.resize(nind); sim_initcsum.resize(nind); 
		logsim_initc.resize(nind); 
		sim_entercsumtot.resize(nind); sim_enterc.resize(nind); sim_entercsum.resize(nind); 
		logsim_enterc.resize(nind); 
		for(i = 0; i < nind; i++){
			...
			sim_enterc[i].resize(ncomps); 
			for(c = 0; c < ncomps; c++){ 
				if(enterpos[c] == 1) sim_enterc[i][c] = 400.0/ncomps;
				else sim_enterc[i][c] = 0;
			}
			sim_entercsum[i].resize(ncomps); logsim_enterc[i].resize(ncomps);
		}
	}
	
  simuo_initcsumtot.resize(naddremdiv+1); simuo_initc.resize(naddremdiv+1);
	simuo_initcsum.resize(naddremdiv+1); logsimuo_initc.resize(naddremdiv+1);
  simuo_entercsumtot.resize(naddremdiv+1); simuo_enterc.resize(naddremdiv+1); 
	simuo_entercsum.resize(naddremdiv+1); logsimuo_enterc.resize(naddremdiv+1); 
  for(a = 0; a <= naddremdiv; a++){
    ...
    simuo_enterc[a].resize(ncomps); 
		for(c = 0; c < ncomps; c++){ 
			if(enterpos[c] == 1) simuo_enterc[a][c] = 400.0/ncomps; 
			else simuo_enterc[a][c] = 0;
		}
    simuo_entercsum[a].resize(ncomps); logsimuo_enterc[a].resize(ncomps);
  }

  if(corona == 0){
		simcl_initcsumtot.resize(nind); simcl_initc.resize(nind); simcl_initcsum.resize(nind); 
		logsimcl_initc.resize(nind);
	  simcl_entercsumtot.resize(nind); simcl_enterc.resize(nind); simcl_entercsum.resize(nind); 
		logsimcl_enterc.resize(nind);
	  for(i = 0; i < nind; i++){
			simcl_initcsumtot[i].resize(clall); simcl_initc[i].resize(clall); 
			simcl_initcsum[i].resize(clall); logsimcl_initc[i].resize(clall);
			simcl_entercsumtot[i].resize(clall); simcl_enterc[i].resize(clall); 
			simcl_entercsum[i].resize(clall); logsimcl_enterc[i].resize(clall);
			for(cl = 0; cl < clall; cl++){
				simcl_initc[i][cl].resize(nclassval[cl]); 
				for(c = 0; c < nclassval[cl]; c++) simcl_initc[i][cl][c] = 400.0/nclassval[cl];
				simcl_initcsum[i][cl].resize(nclassval[cl]); logsimcl_initc[i][cl].resize(nclassval[cl]);
				simcl_enterc[i][cl].resize(nclassval[cl]); 
				for(c = 0; c < nclassval[cl]; c++) simcl_enterc[i][cl][c] = 400.0/nclassval[cl];
...
void Chain::simsumcalc()                         // Used to initialise sampling of initial compartment
{
  ...
  long i;
	...
	if(corona == 0){
		for(i = 0; i < nind; i++){
			sim_initcsumtot[i] = 0;
			for(c = 0; c < ncomps; c++){
				sim_initcsumtot[i] += sim_initc[i][c]; sim_initcsum[i][c] = sim_initcsumtot[i];
			}
			...
			for(c = 0; c < ncomps; c++){ 
				sim_entercsumtot[i] += sim_enterc[i][c]; sim_entercsum[i][c] = sim_entercsumtot[i];
			}
			...
			for(cl = 0; cl < clall; cl++){
				...
				for(c = 0; c < nclassval[cl]; c++){ 
					simcl_initcsumtot[i][cl] += simcl_initc[i][cl][c]; 
					simcl_initcsum[i][cl][c] = simcl_initcsumtot[i][cl];
				}
				if(simcl_initcsumtot[i][cl] == 0){ 
					for(c = 0; c < nclassval[cl]; c++) logsimcl_initc[i][cl][c] = -large;
				}
				else{
					for(c = 0; c < nclassval[cl]; c++){
						logsimcl_initc[i][cl][c] = log(simcl_initc[i][cl][c]/simcl_initcsumtot[i][cl]);
					}
				}

				simcl_entercsumtot[i][cl] = 0;
				for(c = 0; c < nclassval[cl]; c++){ 
					simcl_entercsumtot[i][cl] += simcl_enterc[i][cl][c]; 
					simcl_entercsum[i][cl][c] = simcl_entercsumtot[i][cl];
				}
				if(simcl_entercsumtot[i][cl] == 0){ 
					for(c = 0; c < nclassval[cl]; c++) logsimcl_enterc[i][cl][c] = -large;
				}
				else{
					for(c = 0; c < nclassval[cl]; c++){
						logsimcl_enterc[i][cl][c] = log(simcl_enterc[i][cl][c]/simcl_entercsumtot[i][cl]);
					}
				}
			}
		}
  ...
  for(a = 0; a <= naddremdiv; a++){
    ...
    for(c = 0; c < ncomps; c++){
			simuo_initcsumtot[a] += simuo_initc[a][c]; simuo_initcsum[a][c] = simuo_initcsumtot[a];
		}
    ...
    for(c = 0; c < ncomps; c++){ 
			simuo_entercsumtot[a] += simuo_enterc[a][c]; simuo_entercsum[a][c] = simuo_entercsumtot[a];
		}
    if(simuo_entercsumtot[a] == 0){ for(c = 0; c < ncomps; c++) logsimuo_enterc[a][c] = -large;}
    else{ 
			for(c = 0; c < ncomps; c++){
				logsimuo_enterc[a][c] = log(simuo_enterc[a][c]/simuo_entercsumtot[a]);
			}
		}
  }

  addremdivsumtot = 0;
	for(a = 0; a <= naddremdiv; a++){ addremdivsumtot += addremdiv[a]; addremdivsum[a] = addremdivsumtot;}
  ...
  for(a = 0; a <= naddremdiv; a++){
    induoagesumtot[a] = 0;  
		for(ag = 0; ag < nclassval[agecl]; ag++){
			induoagesumtot[a] += induoage[a][ag]; induoagesum[a][ag] = induoagesumtot[a];
		}
    if(induoagesumtot[a] == 0){ for(ag = 0; ag < nclassval[agecl]; ag++) loginduoage[a][ag] = -large;}
    else{ 
			for(ag = 0; ag < nclassval[agecl]; ag++){
				loginduoage[a][ag] = log(induoage[a][ag]/induoagesumtot[a]);
			}
		}
  }
}
...
void partinit()                               // Initialises quantities used when sampling from particles
{
  ...
  for(cl = 0; cl <= clall; cl++){ 
		compleavesimcldep[cl].resize(ncomp+1); compleavesimclnotdep[cl].resize(ncomp+1);
	}

  for(tr = 0; tr < ntra; tr++){
     if(tra[tr].type == EXP_TR && tra[tr].cf != NOTALIVE && !(tra[tr].capev >= 0 
				&& capevall[tra[tr].capev] == 1) && tra[tr].like == 1){
      eq = tra[tr].eq;
      switch(transdep[eq]){
  ...
  depmax = 0; ndmax = 0;
  ncompleavedep.resize(ncomp+1);
	for(c = 0; c <= ncomp; c++){
		ncompleavedep[c] = compleavedep[c].size();
		if(ncompleavedep[c] > depmax) depmax = ncompleavedep[c];
	}
  ncompleavenotdep.resize(ncomp+1); 
	for(c = 0; c <= ncomp; c++){ 
		ncompleavenotdep[c] = compleavenotdep[c].size(); 
		if(ncompleavenotdep[c] > ndmax) ndmax = ncompleavenotdep[c];
	}
