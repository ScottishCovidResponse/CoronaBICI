...
void Chain::addreminf()
{
	...
	long checkon = 0;
	long l, i, cinit, c, cc, tb, m, d, e, emax, ee, si, p, pr, multac, s, nni;
				...
				if(ran() < al){
					...
					if(checkon == 1 && ran() < 0.1){  // CHECK
						...
						if(m < multac) emsg("prob");
							...
							getprob(cinit);
							for(m = 0; m < multac; m++){
								e = 1; emax = evrevst[m].size(); while(e < emax && tra[evrevst[m][e].tr].cl != 0) e++;
...
{
	long e, ee, tb, d, ci, cf, eq, dst[nsettime+1], s, ddti, tr;
	double sum, t, tt, val;
