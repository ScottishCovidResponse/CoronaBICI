 // This file calculates prior probabilities
...
double Chain::priorsmooth(long s, long j)   // The smoothing prior
{
  double d;
  ...
  switch(smoothtype[s]){
    ...
    case LOGSMOOTH:
      if(param[smoothparam[s][j+1]] < 0 || param[smoothparam[s][j]] < 0){
        stringstream ss; 
				ss << "When log smoothing '" << smoothname[s] << "' cannot have negative parameter value.";
      }
...
double boundprob(double val, string st, string s)
{
  if(simon == 1){
    ...
    stringstream ss;
    if(val < 0){
			ss << "The value " << st << " = " << val << " in the " << s << " distribution is negative.";
		}
    else ss << "The value for " << st << " in the " << s << " distribution is too small.";
    emsg("Prior: EC2 "+ss.str());
  }
  else sampill = 1;
...
double Chain::priorsamp(long pr)           // Samples from the prior
{
  ...
  switch(priortype[pr]){ 
    ...
    case GAMMA:
      ...
      if(alpha < gammalim || val == 0){
        if(simon == 1) emsg("Prior: EC3 The shape parameter for the gamma distribution is too small");
    ...
    case LOGNORMAL:
      ...
      if(val == 0 || isnan(val)){
        if(simon == 1) emsg("Prior: EC4 The value for the log-normal distribution is out of range.");
        else{ sampill = 1; return 1;}
      }
      return val;
    ...
    case BETA:
      ...
      if(val == 0 || val == 1 || isnan(val)){
        if(simon == 1) emsg("Prior: EC5 The value for the beta distribution is out of range.");
        else{ sampill = 1; return 1;}
      }
      return val;
    ...
    case WEIBULL:
      ...
      if(val == 0 || isnan(val)){
        if(simon == 1) emsg("Prior: EC6 The value for the Weibull distribution is out of range.");
        else{ sampill = 1; return 1;}
      }
      return val;
...
double Chain::priorprob(long pr)                 // Calculates the prior probability
{
  ...
  switch(priortype[pr]){
    ...
    case FLAT:
      ...
      return log(1.0/(priormaxval[pr]-priorminval[pr]));
      break;
...
void addparampriordep(long p, long pr)    // Adds a prior depdendency for a parameter
{
  long j;
...
void priorinit()                          // Initialisation of the prior
{
  long pr, eq, p, j;
  double dt;
  ...
  for(p = 0; p < nparam; p++) nparampriordep.push_back(parampriordep[p].size());
	...
	// A prior such that the prior probability of being in any one of the age classifications is the same
  agprior.resize(nclassval[agecl]);   
