// This file makes local changes to individual event sequences
...
void Chain::sing_prop(long i)        // Swaps between single and two events
{
  ...
  e = long(ran()*(nev-1));
  if(e == 0) cl = long(ran()*(nclass-3)); 
	else{ cl = tra[indev[i][e].tr].cl; if(cl >= nclass-3) return; if(cl < 0) emsg("Local: EC1");}
...
void Chain::twothree_prop(long i)             // Interchanges between two and three events
{
  ...
  e = long(ran()*(nev-1));
  if(e == 0) cl = long(ran()*(nclass-3)); 
	else{ cl = tra[indev[i][e].tr].cl; if(cl >= nclass-3) return; if(cl < 0) emsg("Local: EC2");}
...
void Chain::pair_prop(long i)       // Interchanges a pair of events
{
  long e, ee, nev, cl, fl;
...
long Chain::localresamp(long i, long cl, long estart, double t, double tend, long type, long evfac)  
{
  ...
  while(eend < nev && indev[i][eend].t <= tend){
    tr = indev[i][eend].tr;
    if(tra[tr].cl == cl || tra[tr].cl < 0){ if(fl == 0) fl = 1;
		else inter.push_back(compval[tra[tr].ci][cl]);}
    eend++; c = tra[tr].cf;
  }
  ...
  switch(type){
    case 1:   // When there are events at the begining and end
      switch(numi){
        ...
        case 3:
          ...
          default: emsg("Local: EC6"); return -1;
      }
      break;
    ...
    case 0:
      switch(numi){
        ...
        case 2:
          if(ii == ff){
            n = nlocpath[cl][ii][ff][0]; if(n != 1) emsg("Local: EC10");
            ...
            n = nlocpath[cl][ii][ff][1]; if(n == 0) return -1; if(n != 1) emsg("Local: EC11");
            fac /= nlocpath[cl][ii][ff][2];
  ...
  if(nevadd != 0 && locpathst[p].size() != nevadd-1) emsg("Local: EC17");
  evnew.clear();
  e = estart;
  ji = ii;
  for(k = 0; k <= nevadd; k++){
    ...
    while(e < nev && indev[i][e].t <= tt){
      ...
      if(tra[tr].cl != cl && tra[tr].cl >= 0){
        cf = ci + tra[tr].dc; tr2 = compiftra[ci][cf]; if(tr2 < 0) emsg("Local: EC18"); 
    ...
    if(ji == nval){ 
      if(estart != 0) emsg("Local: EC19"); 
			cf = tra[indev[i][0].tr].cf; cf = cf + (jf-compval[cf][cl])*classmult[cl]; 
    ...
    if(tr < 0){ if(tr != -1) emsg("Local: EC20"); return -1;}
    if(fixfl == 1){ 
			if(tra[tr].capev >= 0 && capevall[tra[tr].capev] == 1 && tt > t && t < tend) return -1;
		}
  ...
  al = exp(Ltotf-Ltoti)*fac; 
	if(plfl == 1) cout << estart << " " <<  al << " " << fac << " " << Ltotf << "  "<< Ltoti<<  "al\n\n\n";
...
void localpropinit()           // Initialisation of local proposals
{
  ...
  for(tr = 0; tr < ntra; tr++){
    cl = tra[tr].cl;
    j = 0; 
		while(j < ntrans && (trans[j].cl != cl || trans[j].i != tra[tr].i || trans[j].f != tra[tr].f)) j++;
    tra[tr].trans = j;
    if(j == ntrans){
			TRANS tran; tran.cl = cl; tran.i = tra[tr].i; tran.f = tra[tr].f; trans.push_back(tran); ntrans++;
		}
  }
  ...
  for(cl = 0; cl < nclass-3; cl++){
    ...
    for(ii = 0; ii <= nval; ii++){
      ...
      for(ff = 0; ff <= nval; ff++){
        ...
        j = 0;
				if(nlocpath[cl][ii][ff][0] > 0) j++; 
				if(nlocpath[cl][ii][ff][1] > 0) j++; 
				if(nlocpath[cl][ii][ff][2] > 0) j++;
