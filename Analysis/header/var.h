...
vector <double> prob;               // Used to sample infection time
...
const long SIMTIME = 25, SINGFIX_PROP = 26;
...
vector <vector <EV> > evnothing;               // Event sequences when nothing happens
...
long ist, nwrong, n_popnum, sampill, dcsett;
...
class Chain                                    // Store all quantities and funcion for an MCMC chain
{
  public:
    ...
    vector< vector <double> > logsimuo_enterc, simuo_enterc, simuo_entercsum;         
		...
		vector <vector <long> > obsinflist; // List of observed individuals
		vector <vector <long> > inflist;    // List of unobserved infected individuals
		vector <vector <long> > notinflist; // List of unobserved uninfected individuals
		
		// Used for singlefixed proposal
		vector <float> ntr_singfix, nac_singfix;	
		vector <float> sificha;
		...
		// Used for sampling assuming a hospitalised case
		vector <double> samp_r;
		vector <long> samp_nleave;
		vector< vector <long> > samp_leave;
		vector< vector <double> > samp_prob;
		
		vector <double> samprev_r;
		vector <long> samprev_enter;
  ...
  double probuocap(long i, vector<EV> &vec);          
	...
	void savesimulated();
	void loadsimulated();
	double simfromSE(long i, double t);
	double simfrom(long c, double t);
	double simfromfixed(long i);
	
	double probsimfrom(vector<EV> &vec, long e);
