...
void Chain::initinflist()  // Initialises the list of observed /infected / noinfected individuals (Corona)
{
	...
	inflist.resize(nregion); obsinflist.resize(nregion); notinflist.resize(nregion);
	for(r = 0; r < nregion; r++){ 
		inflist[r].resize(nag); obsinflist[r].resize(nag); notinflist[r].resize(nag);
	}
	
	indinflist.resize(nind);
	for(i = 0; i < nind; i++){
		r = (indinit[i]/nclassval[0])%nregion;
		a = (indinit[i]/(nclassval[0]*nclassval[1]))%nag;
		
		if(nindev[i] > 2+nsettime){
			indinflist[i] = obsinflist[r][a].size();
			obsinflist[r][a].push_back(i);
		}
		else{
