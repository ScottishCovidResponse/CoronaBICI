...
double Chain::simfromfixed(long i)  // Simulated around a single fixed event 
{
	...
	for(k = cst.size()-1; k >= 0; k--){
		...
		while(stnext < t){ 
			cs = c+dcsett;
			evm.tr = compiftra[c][cs]; evm.t = stnext; evnew.push_back(evm);
			c = cs; s++; 
			stnext = settime[s];
		}
...
void Chain::samplerinit()  // Generates the samplers used when adding individuals or during initialisation
{
	...
	for(cinit = 0; cinit < ncompswa; cinit++){
		...
		for(s = 0; s < nsettime; s++){
			...
			EV evm; 
			evm.tr = compiftra[c][cc];
			evm.t = settime[s]; 
			evnothing[cinit].push_back(evm);
			c = cc;
		}	
