// This file considered proposals affecting the lifespan of an individual

void Chain::life_prop(long i)    // Makes changes to entry and leaving times as well as time of birth
{
  long type, fl, elea, nev, e;
  double p, tbirth, tnew, facup = 1.04, facdown = 0.98, facup2 = 1.004, facdown2 = 0.998;
	double	dt, tlimmin, tlimmax, fot, lot;

  p = ran(); type = 0; while(type <  7 && p > problifepropsum[type]) type++;
	if(type == 7) emsg("Life: EC1");
  ...
  switch(type){
    ...
    case 1:    // Change leave time
      ...
      nev = nindev[i];
      e = elea+1; while(e < nev && tra[indev[i][e].tr].cl >= nclass-3) e++;
			if(e < nev) tlimmax = indev[i][e].t; else tlimmax = large;
			
      e = elea-1; while(e >= 0 && tra[indev[i][e].tr].cl >= nclass-3) e--; 
    ...
    case 2:    // Change enter time
      ...
      nev = nindev[i];
      e = 1; while(e < nev && tra[indev[i][e].tr].cl >= nclass-3) e++; 
			if(e < nev) tlimmax = indev[i][e].t; else tlimmax = large;

      if(i < nind) fot = firstobst[i]; else fot = large;
      if(tnew < 0 || tnew > tmax || (tnew > tent && tnew > tlimmax) 
				|| tnew < indtbirth[i] || tnew > fot || tnew > indtbirth[i]+age[nage]) fl = 0;
    ...
    case 3:    // Change birth time
      ...
      if(i < nind) tbirth = indtbirth[i] + normal(0,jump_tbirth[i]); 
			else tbirth = indtbirth[i] + normal(0,jump_tbirthuo);
    ...
    case 4:    // Switch enter
      if(i < nind && indfixtenter[i] != large) return;
      if(indtbirth[i] <= 0){
       nev = nindev[i]; e = 1; while(e < nev && tra[indev[i][e].tr].cl >= nclass-3) e++; 
			 if(e < nev) tlimmax = indev[i][e].t; else tlimmax = large;
...
// Makes a change to the lifespan
long Chain::lifechange(long i, double tentnew, double tleanew, double tbirthnew, double dprob) 
{
  ...
  tr = indev[i][0].tr;
  if(tbirthfl == 1){ 
		agold = compval[tra[tr].cf][agecl]; dLpri = agprior[ag] - agprior[agold];
	}
	else dLpri = 0;
  ...
  while(ti < nsettime && settime[ti] < tleanew){
    EV ev; ev.t = settime[ti]; ev.tr = compiftra[ti*classmult[settimecl]][(ti+1)*classmult[settimecl]]; 
		evnew.push_back(ev);
    ti++;
  }

  while(f < ncapevtrange && capevtrange[f] < tleanew){
    EV ev; ev.t = settime[ti]; ev.tr = compiftra[f*classmult[capevcl]][(f+1)*classmult[capevcl]]; 
		evnew.push_back(ev);
    f++;
  }
  ...
  al = exp(Ltotf - Ltoti + dprob + dLpri);
