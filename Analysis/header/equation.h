// This file converts strings representing equations into calculations

// Diferent elements to an equation:
const long LEFTBRACKET = 0, RIGHTBRACKET = 1, ADD = 4, TAKE = 5, MULTIPLY = 6;                
...
vector<long> neqncalc;                              // The number of basic operations to calculate an eq
...
vector< vector<long> > eqncalcfastnum1;             // Look up table rto easily calculate population number
...
long popcombine(long i);                            // Works out population can be combined to others
...
long addequation(string s)                          // Adds a new equatrion to the list
{
  ...
  i = 0;                                          // Copes with a negate minus sign
  while(i < s.length()){
    if(s.substr(i,1) == "-"){
  ...
  while(j < long(s.length())-1){
    if(s.substr(j,1) == "{"){
      ...
      if(j == s.length()) emsg("Equation: EC1");
  ...
  while(i < len){ 
    ...
    switch(cstr[0]){
      ...
      default:
        ...
        if(doneflag == 0) emsg("Equation: EC2 Problem with expression");
        break;
    }
    i++;
  }
  ...
  while(i < long(op.size())-1){
    if((op[i] == PARAM || op[i] == POPNUM || op[i] == RIGHTBRACKET) &&
        (op[i+1] == PARAM || op[i+1] == POPNUM || op[i+1] == EXPFUNC || op[i+1] == SINFUNC 
				|| op[i+1] == COSFUNC || op[i+1] == LOGFUNC ||  op[i+1] == STEPFUNC || op[i+1] == LEFTBRACKET)){
  ...
  if(plfl == 1) printop();
	...
	i = 0;   // Combines populations with numerical prefactors
	while(i < long(op.size())-3){
		if(op[i] == POPNUM && op[i+1] == MULTIPLY && op[i+2] == NUMERIC){ 
			popcomb[
		}
		...
		popcomb = opnum[i]; popcombnum = numeric[opnum[i+2]];}
	}
	
	}
	*/
	
  i = 0;                                 // Looks to combine together multiple population expressions
  while(i < long(op.size())-3){
    if(popcombine(i) == 1){
  ...
  if(op.size() != 1) emsg("Equation: EC3 Problem with expression");
...
long popcombine(long i)                        // Works out if a population can be combined with others
{
  ...
  if(op[i] == NUMERIC && op[i+1] == MULTIPLY && op[i+2] == POPNUM){
		popcomb = opnum[i+2]; popcombnum = numeric[opnum[i]];
	}

  if(op[i] == POPNUM && op[i+1] == DIVIDE && op[i+2] == NUMERIC){ 
		popcomb = opnum[i]; popcombnum = 1.0/numeric[opnum[i+2]];
	}
...
void printcalc(long eq)                             // Outputs a calculation
{
  long i;
  ...
  for(i = 0; i < neqncalc[eq]; i++){
    if(eqncalcop[eq][i] == ADD || eqncalcop[eq][i] == TAKE || eqncalcop[eq][i] == MULTIPLY 
			|| eqncalcop[eq][i] == DIVIDE || eqncalcop[eq][i] == POWER){
...
double calculate(long eq, double *popnum, double *paramval)          // Calculates an equation
{
  long i;
  ...
  for(i = 0; i < neqncalc[eq]; i++){
    ...
    switch(eqncalcop[eq][i]){
      ...
      case DIVIDE: if(num2 == 0) emsg("Equation: EC5 A division by zero!"); num = num1/num2; break;
...
double calculatenotdep(long eq, double *param)              // Calculates an equation with no dependency
{
  long i;
  double num, num1, num2;
  ...
  for(i = 0; i < neqncalc[eq]; i++){
    ...
    switch(eqncalcop[eq][i]){
      ...
      case TAKE: num = num1-num2; break;
      case MULTIPLY: num = num1*num2; break;
      case DIVIDE: if(num2 == 0) emsg("Equation: EC10 A division by zero!"); num = num1/num2; break;
      case POWER: num = pow(num1,num2); break;
      case EXPFUNC: num = exp(num2); break;
      case SINFUNC: num = sin(num2); break;
      case COSFUNC: num = cos(num2); break;
...
vector<long> getallcomp(string st)                      // Gets all the comprtments from a string
{
  ...
  for(k = 0; k < vec.size(); k++){
    ...
    if(cl == nclass) emsg("Equation: EC14");
  }
