function inferencebuts()                                  // Intialises the buttons on the inference page
{
	...
	case "Dependency":
		...
		de +=" against "+depopsel;
		addbutton(de,x,30,0,0,-1,TITLEBUT,54,-1);
		showhisto(); 
		break;
	...
	case "Trans. Dist.":
		...
		addbutton(de,x,30,0,0,-1,TITLEBUT,53,-1);
...
function drawsetup()                                     // Page showing inference setup options
{
	...
	if(corona == 1){
		addbutton("Load",width-105-100,height-45,90,30,STARTAC,ADDDATABUT,0,1);
		addbutton("Init",width-105,height-45,90,30,STARTAC,GREENBUT,0,-1);
	}
	else{
...
function drawadvop()                                     // Page for advanced setup options
{
	...
	addcanbutton("Sample limits",x,y,150,0,-1,TEXTBUT,-1,-1); y += 25;
	...
	addcanbutton("Individual limits",x,y,150,0,-1,TEXTBUT,-1,-1); y += 30;
...
function loadanalysis(st)
{
	var lines = st.split('\n'), li;
	for(li = 0; li < lines.length-2; li++){
		if(lines[li] != "") processline(lines[li],0);
	}
	loading = 0;
	...
	case "inf":
		infres.result = 2;
		changepage(INFERENCEPAGE,2,10);
		break;
	case "sim":
		simres.result = 1; gentype = -1;
		changepage(SIMULATEPAGE,2,2); break;
		break;
	}
}
