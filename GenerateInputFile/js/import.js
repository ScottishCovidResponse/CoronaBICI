...
function importfile(text)                                 // Import a script to define (or partially define) model and data 
{
	...
	for(j = 0; j < lines.length; j++){
		...
		if(trr != ""){
			...
			//for(n = 0; n < tags.length; n++){ pr(tags[n]);}
				
			switch(type){
			case "map":
				var mapX = gettag("X"), mapY = gettag("Y");
				var mapkey = gettag("key").split(",");
				st = gettag("table");
				lines = st.split("\n");
				var mapgrid=[];
				for(j = 0; j < lines.length; j++){
					mapgrid[j]=lines[j].split(",");
				}
				map = {X:mapX, Y:mapY, key:mapkey, grid:mapgrid};
				alert("Map Imported");
				break;
			...
			case "compartment":
				...
				if(cl == ncla-2) addclass(claname);
					...
					if(clcompch[cl] != 1){
						while(cla[cl].ncomp > 0) deletecomp(cl,0);
						collectvariables();
					}
				}
				clcompch[cl] = 1;
				...
				checkname(name,"");
				if(errmsg != ""){
					alertimp("For compartment '"+name+"': "+errmsg);
					return;
				}
