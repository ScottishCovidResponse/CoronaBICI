...
function drawmodel(cl,xco,yco,dx,dy,frame,type)            // Draws all the elements of the model
{
	...
	if(type == "indoutput"){
		...
		var stax = 0, stay = 0, nsta = 0, endx = 0, endy = 0, nend = 0;
		
		for(ch = chmin; ch < chmax; ch++){
			for(s = smin; s < smax; s++){
				w = res.ch[ch].sampev[s].ind[i];
				
				frx = 0.5 + ch*3.5343+ s*5.534; fry = 0.5 + ch*5.5343 + s*4.244;
				frx -= Math.floor(frx); fry -= Math.floor(fry);
				...
				for(e = 0; e < w.nev; e++){
					...
					if(cc != NOTALIVE){
						...
						x = xco+(c.x+frx*c.w-shx)*shfac; y = yco+(c.y+fry*c.h-shy)*shfac;
						
						if(e == 0){ stax += x; stay += y; nsta++;}
						if(e == w.nev-2){ endx += x; endy += y; nend++;}
						
						p.push({x:x, y:y, t:w.evt[e]});
					}
				}
				...
				for(e = 0; e < p.length-1; e++){
					if(smax == smin+1){ 
						...
						drawgradientline(p[e].x,p[e].y, p[e+1].x,p[e+1].y,(p[e].t- p[0].t)/dt,(p[e+1].t- p[0].t)/dt,1);
					}
				}
			}
		}
		...
		r = 5;
		x = Math.floor(stax/nsta); y = Math.floor(stay/nsta);
		centertext("Start",x,y-9,"bold 16px Times",BLACK); 
		if(nsta == 1) fillcircle(x,y,r,BLUE,BLACK,2);
		
		x = Math.floor(endx/nend); y = Math.floor(endy/nend)
		centertext("End",x,y-9,"bold 16px Times",BLACK); 
		if(nend == 1) fillcircle(x,y,r,RED,BLACK,2);
	}
}
...
{
	var fosize = "20px Times", fody=0, fosubsize = "12px Times", fosubdy= 4, loop, loopmin, pa = -8, pa2 = -19;
	
	//fosize = "30px Times"; fody=0; fosubsize = "18px Times"; fosubdy=6; pa = -10; pa2 = -25;
	...
	if(page == INFERENCEPAGE){
		col = LGREY;
		if(pagesub[page] == 0){ col = BLACK; if(ov == 1) col = RED;}
	}
	...
	if(tr.i != -1 && tr.f != -1 && tr.l*shfac > 40){	
		...
		case "Exponential": case "Source": case "Sink": case "Gamma": case "Weibull": case "Fixed":
			for(j = 0; j < tr.ratevar.length; j++){
				stsmall += tr.ratevar[j]; if(j < tr.ratevar.length-1) stsmall += ",";
			}
			break;
		}
		...
		if(stbig != ""){
			...
			if(w < 0.9*tr.l*shfac){
				...
				for(j = 0; j < lab.length; j++){
					if(tr.middx > 0){
						plotangletext(lab[j].frag,midx+(lab[j].w-w/2)*tr.middx+(pa+lab[j].dy)*nxmid,
											      midy+(lab[j].w-w/2)*tr.middy+(pa+lab[j].dy)*nymid,th,lab[j].font,col);
					}
					else{
						plotangletext(lab[j].frag,midx+(-lab[j].w+w/2)*tr.middx+(pa2-lab[j].dy)*nxmid,
											      midy+(-lab[j].w+w/2)*tr.middy+(pa2-lab[j].dy)*nymid,th+Math.PI,lab[j].font,col);
					}
				}
			}
		}
...
function getratelines(st)                               // Converts rate expression into lines
{
	...
	for(li = 0; li < lines.length; li++){
		temp = lines[li].split(":");
		switch(temp.length){
		case 1: lines[li] = eqntidy(temp[0]); break;
		case 2: lines[li] = temp[0]+":"+eqntidy(temp[1]); break;
		}
	}
...
function checkeqn(st)                              // Checks the validity of an equation
{
	...
	for(li = 0; li < lines.length; li++){
			...
			if(st.substr(ii,1) == "["){
				...
				if(nnn == nvarlist){
					...
					if(jj == j) vardeplist[nvarlist] = "";
						...
						// Replaces compartments with classifications
						var lin = st.substr(jj+1,j-(jj+1)).split(",");
						var reco="";
						for(jjj = 0; jjj < lin.length; jjj++){
							stt = lin[jjj];
							for(cl2 = 0; cl2 < ncla; cl2++){
								if(stt == cla[cl2].name) break;
								for(c = 0; c < cla[cl2].ncomp; c++){ if(stt == cla[cl2].comp[c].name) break;}
								if(c < cla[cl2].ncomp) break;
							}
							if(cl2 < ncla){
								if(reco != "") reco += ","; reco += cla[cl2].name; 
							}
							else{
								errmsg = "Cannot find dependency '"+stt+"'"; return;
							}
						}
						vardeplist[nvarlist] = reco;
					}						
					nvarlist++;
				}
					...
					for(k = 0; k < an.length; k++){
						for(cl2 = 0; cl2 < ncla; cl2++){
							...
							for(c = 0; c < cla[cl2].ncomp; c++){
								if(cla[cl2].comp[c].name == an[k]){ cladepflag[cl2] = 1; break;}
							}
							if(c < cla[cl2].ncomp) break;
						}
						if(cl2 == ncla){ 
							errmsg = "'"+an[k]+"' is not a classifcation or compartment name"; return;
						}
					}
				}
		...
		if(st != "[&q]"){
			errmsg = "There is a problem with this expression: "+lines[li]; return;
		}
	}
}
