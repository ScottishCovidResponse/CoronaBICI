function init()                                           // Intialises the software
{
  ...
  resultcv = resultcan.getContext('2d');
	...
	//xmlhttp.open("GET","Examples/EX30.bici",true); xmlhttp.send(); 
...
function initpages()                                       // Initialises all the pages and scrollbars
{
	...
	for(i = 0; i < 8; i++){ pagesub[i] = 0; pagesubsub[i]=[]; for(j = 0; j < 3; j++) pagesubsub[i][j] = 0;}
	for(i = 0; i < 8; i++){ pageyst[i]=[]; for(j = 0; j < 12; j++){ pageyst[i][j]=[]; for(k = 0; k < 12; k++) pageyst[i][j][k]=0;}}	
}
